...
*Jul 15 2026 — Isaac Tai, Daniel Kim, Mike Gao*
...
*[Figure: Exploded vertical stack of the internal knowledge base — Sources feed distillation, embeddings, retrieval, fusion, and synthesis layers.]*
...
*[Figure: Many data sources feeding one queryable embeddings table — Slack, Wiki/Confluence, Code Repos, Netlists, PRM Docs, Custom Databases → one embeddings table (document, embedding, metadata, source + timestamps) → query via MCP, Web UI, Agents]*
...
*[Figure: Slack event flow from Socket Mode through distillation and embedding — Socket Event → Route → (@mention/DM → Bot Reply; tracked channel → Reingest Thread → Upsert Thread) → Sync Worker → Distill / Thread Vector / Burst Vectors, with reset watermarks]*
...
*[Figure: worked example of the four scoring signals against a sample query "restore hangs after manifest load"]*

- **2w ago, thread** — "Checkpoint stalls on the NFS mount — set CKPT_PREFETCH=4." — No shared tokens; paraphrase match; CKPT_PREFETCH is rare; recent wins tie.
- **1d ago, message** — "ERR_MANIFEST_TIMEOUT: restore hangs after manifest load." — Exact tokens; ERR_MANIFEST_TIMEOUT is rare.
- **3h ago, message** — "sounds good, thanks! will try that" — False neighbor; no rare tokens.
- **8mo ago, thread** — "restore hangs after manifest load → use LEGACY_FETCHER=1." — Also matches, but decayed (8 months old).
...
*[Figure: worked example of thread distillation — a sample thread annotated by role]*
...
- **Vector (pgvector)** — Ready: one consistent record, 3,072 dimensions — built from question + summary + resolution + systems + code references.
...
*[Figure: A Slack thread split into same-author bursts and filtered by signal]*
...
*[Figure: Planner, parallel tool execution, evidence normalization, and synthesis]*
...
*[Figure: MCP exposes retrieval primitives while the web UI runs the complete agent pipeline]*
...
*[Figure: Projects reference shared data sources without duplicating them]*
