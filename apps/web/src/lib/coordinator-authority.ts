...
// `announceApprovalDecision`, which returns words and consumes none.
//
...
// so every rule above is falsifiable in text.
 ...
 * receipt, no outcome" rule stays enforced in one place. The signature is
 * one-directional on purpose: a decision goes in, words come out, and nothing
 * a listener or a model says can travel back the other way.
 */
...
function narrowActionRequest(value: unknown): DelegatedActionRequest | undefined {
  ...
  if (!Object.keys(candidate).every((key) => allowed.has(key))) return undefined

  const { actionId, action, capability, principalId, resourceScope } = candidate
