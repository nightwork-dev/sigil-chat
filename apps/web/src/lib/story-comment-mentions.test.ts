...
describe("story comment selectors", () => {
  it("accepts public role terms and open validated handles", () => {
    expect(parseSingleInlineSelector("Please check this, @coordinator.")).toBe(
      "coordinator",
    )
    expect(
      parseSingleInlineSelector("For @review-agent_2 when available"),
    ).toBe("review-agent_2")
    expect(parseSingleInlineSelector("For @review.agent when available")).toBe(
      "review.agent",
    )
  })
  ...
  it("does not route email addresses or distinct multi-target mentions", () => {
    expect(
      parseSingleInlineSelector("mail person@example.test"),
    ).toBeUndefined()
    expect(
      parseSingleInlineSelector("@coordinator and @analysis"),
    ).toBeUndefined()
    expect(parseSingleInlineSelector("@analysis, then @analysis")).toBe(
      "analysis",
    )
  })
