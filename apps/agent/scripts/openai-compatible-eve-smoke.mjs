...
try {
  ...
  const streamSummary = await assertToolLoopStream({
    expectedFinalText: "local smoke complete",
  ...
  console.log(
    `Eve local OpenAI-compatible smoke passed: POST /eve/v1/session reached the official provider adapter, exposed the native todo tool, executed it, sent the tool result back to the model, and completed the stream (${streamSummary.events} events)`,
  )
} catch (error) {
