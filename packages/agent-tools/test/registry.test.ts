...
async function makeRegistry(
  ...
  scopedKnowledge?: {
    ...
    tools?: ReturnType<typeof scopedKnowledgeTools>
  },
) {
  ...
  return {
    registry: createSigilAgentToolRegistry({
      artifacts:
        artifacts ??
        createFileSessionArtifactStore({ root: join(directory, "artifacts") }),
      ...
      ...(scopedKnowledge
        ? {
            ...
            scopedKnowledgeTools: (
              scopedKnowledge.tools ?? scopedKnowledgeTools()
            ).map((tool) => bindScopedKnowledgeToTool(tool, scopedKnowledge.store)),
          }
        : {}),
      skills:
        skillRegistry ??
...
describe("Sigil Chat Gonk registry", () => {
  ...
  it("lists and reads only files in the caller's session scope", async () => {
    ...
    const listed = await collectToolOutcome(
      registry.invoke(
        ...
        {},
        makeBaseContext({ host: { resourceScope: "session:thread-a" } }),
      ),
    );
    ...
    const otherSession = await collectToolOutcome(
      registry.invoke(
        "sigil-list-session-files",
        {},
        makeBaseContext({ host: { resourceScope: "session:thread-b" } }),
      ),
    );
  ...
  it("discovers a personal agent's live readable resource universe and re-authorizes retrieval", async () => {
    ...
    const artifacts = createFileSessionArtifactStore({
      root: directory,
      canAccessScope: (principal, scope) =>
        principal?.id === "user-1" && readable.has(`${scope.tier}:${scope.id}`),
    });
  ...
  it("edits a scoped source image into a new artifact with provenance", async () => {
    ...
    const outcome = await collectToolOutcome(
      registry.invoke(
        ...
        makeBaseContext({ host: { resourceScope: "session:thread-image" } }),
      ),
    );
    ...
    const files = await artifacts.listByScope("session:thread-image");
  ...
  it("keeps derivation identity and provenance when edited bytes deduplicate", async () => {
    ...
    const source = await artifacts.putFile({
      ...
      filename: "portrait.png",
      mediaType: "image/png",
      scope: "session:thread-image-dedupe",
    });
    const registry = new ToolRegistry({
      security: { approvalProvider: sigilApprovalProvider },
    });
    ...
    const first = await collectToolOutcome(
      registry.invoke(
        ...
        makeBaseContext({
          host: { resourceScope: "session:thread-image-dedupe" },
        }),
      ),
    );
    ...
    const files = await artifacts.listByScope("session:thread-image-dedupe");
  ...
  it("persists an inline edit source and fails loudly without a backend", async () => {
    ...
    const outcome = await collectToolOutcome(
      registry.invoke(
        ...
        makeBaseContext({ host: { resourceScope: "session:thread-inline" } }),
      ),
    );
    ...
    const files = await artifacts.listByScope("session:thread-inline");
